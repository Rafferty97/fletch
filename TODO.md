...
- [ ] **Fix `type_map`**: currently `FnvHashMap<NodeId, Ty<'a>>` — this puts arena
  ...
  - prefer `Repr` (physical class: i32/i64/f64/bool/string/list/struct +
    nullability) if instruction selection only branches on repr — arena drops
    at the check->lower boundary, `Compiler<'a>` keeps only the symbol borrow.
  - fall back to `TyId` + frozen type interner only if lowering re-runs
    structural queries on types.
- [ ] Binding -> slot map: `FnvHashMap<DefId, Slot>`, filled during allocation.
      (Not `Vec<DefId>`, not slot-identity — scoped slot reuse voids any 1:1.)
- [ ] Slot allocation: stack discipline — reset the slot counter on scope exit
...
- [ ] Weeder pass — only if non-local checks accumulate (`break`/`return`
      ...
      assignment target.
- [ ] Slot packing via real liveness (interleaved live ranges within a scope) —
      only if frame size shows up in profiling. Reintroduces a real `DefId->Slot`
      map; opt-in.
- [ ] `DefId -> Slot` is currently global-keyed; revisit only if incremental
      recompilation ever becomes a goal.
