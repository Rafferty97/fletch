...
pub fn variance_in_ty(ty: &Ty, var: VarId) -> Variance {
    match ty {
        ...
        Ty::Func(func) => {
            let param_variance = variance_in_tys(&func.params, var);
            let ret_variance = variance_in_ty(&func.ret, var).invert();
...
pub fn join(a: &Ty, b: &Ty) -> Ty {
    match (a, b) {
        ...
        (Ty::Nullable(a), b) => Ty::Nullable(join(a, b).into()).into(),
        // todo: tuple, func
        _ => Ty::Top,
    }
}
...
impl InferCtx {
    ...
    pub fn solve_var(&self, var: VarId, ret_ty: &Ty) -> Ty {
        // variance-directed bound selection
        todo!()
    }

    pub fn solve_ty(&self, ty: &Ty, ret_ty: &Ty) -> Ty {
        // substitute all vars recursively
        todo!()
    }
}
