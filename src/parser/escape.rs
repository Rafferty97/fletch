...
impl UnescapeError {
    fn new(pos: usize, kind: UnescapeErrorKind) -> Self {
        let pos = pos.try_into().expect("string too long");
        Self { pos, kind }
    }
}
...
pub fn unescape(raw: &str) -> Result<String, UnescapeError> {
    ...
    while i < bytes.len() {
        ...
        if let Some(backslash_pos) = bytes[i..].iter().position(|&b| b == b'\\') {
            ...
            if backslash_pos > 0 {
                result.push_str(std::str::from_utf8(&bytes[i..absolute_pos]).expect("input should be valid UTF-8"));
            }
            i = absolute_pos;
        } else {
        ...
        i += 1;
        if i >= bytes.len() {
            return Err(UnescapeError::new(i - 1, UnescapeErrorKind::IncompleteEscape));
        }
        ...
        match bytes[i] {
            ...
            b'x' => {
                ...
                i += 1;
                if i + 1 >= bytes.len() {
                    return Err(UnescapeError::new(i - 2, UnescapeErrorKind::IncompleteEscape));
                }
                ...
                let hex_str = std::str::from_utf8(&bytes[i..i + 2]).map_err(|_| {
                    UnescapeError::new(
                        ...
                        UnescapeErrorKind::InvalidEscape {
                            sequence: format!("\\x{}", String::from_utf8_lossy(&bytes[i..i.min(i + 2)])),
                        },
                    )
                })?;
            ...
            b'u' => {
                ...
                while i < bytes.len() && bytes[i] != b'}' {
                    if !bytes[i].is_ascii_hexdigit() {
                        return Err(UnescapeError::new(
                            start - 3,
                            UnescapeErrorKind::InvalidEscape {
                                sequence: format!("\\u{{{}}}", String::from_utf8_lossy(&bytes[start..i + 1])),
                            },
                        ));
                    }
                    i += 1;
                ...
                if i >= bytes.len() {
                    return Err(UnescapeError::new(start - 3, UnescapeErrorKind::UnexpectedEnd));
                }
                ...
                let hex_str = std::str::from_utf8(&bytes[start..i]).map_err(|_| {
                    UnescapeError::new(
                        start - 3,
                        UnescapeErrorKind::InvalidEscape {
                            sequence: format!("\\u{{{}}}", String::from_utf8_lossy(&bytes[start..i])),
                        },
                    )
                })?;
                ...
                if hex_str.is_empty() || hex_str.len() > 6 {
                    return Err(UnescapeError::new(
                        start - 3,
                        UnescapeErrorKind::InvalidEscape { sequence: format!("\\u{{{}}}", hex_str) },
                    ));
                }
            ...
            _ => {
                return Err(UnescapeError::new(
                    i - 1,
                    UnescapeErrorKind::InvalidEscape { sequence: format!("\\{}", bytes[i] as char) },
                ));
            }
        }
    }
